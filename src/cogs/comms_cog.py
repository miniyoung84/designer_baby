...
class CommsCog(commands.Cog):
    ...
    async def ping(self, message):
        ...
        pings = ["p", "ping", ";p"]
        mod_ping = message.content.lower() in pings
        player_ping = message.content.lower() in ["aa", ";a"]
        ...
        if not is_ping and in_ic_channel:
            async for msg in message.channel.history(limit=5):   # Cleanup on leftover pings
                if myid in msg.content or pid in msg.content or msg.content.lower() in ["ping", "p", "aa"]:
                    await msg.delete()
