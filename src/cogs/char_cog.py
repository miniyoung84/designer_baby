...
class CharacterCog(commands.Cog):
    ...
    async def build_embed(self, name: str, color: str, subjects: list, grades_value: list) -> Embed:
        grade_iter = 0
        grade_list = ""
        subject_list = ""
    ...
    async def char(self, ctx, character: str):
        ...
        embed = await self.build_embed(name=full_character, color=rows[0][3], subjects=[row[4] for row in rows], grades_value=[row[5] for row in rows])
